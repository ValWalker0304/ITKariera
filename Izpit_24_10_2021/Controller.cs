...
namespace Izpit_24_10_2021_1
{
    class Controller
    {
        ...
        public string ProcessSettleCommand(List<string> args)
        {
            ...
            Villages.FirstOrDefault(x => x.VillageName == villageName).AddPeasant(peasant);
        ...
        {
            var resources = Villages.FirstOrDefault(x => x.VillageName == args[0]).PassDay();
        ...
        public string ProcessAttackCommand(List<string> args)
        {
            ...
            Villages.FirstOrDefault(x => x.VillageName == args[1]).Resource -= rebelAttackDamage;
            var rebelsCount = (int)(rebelAttack.Count / 2);
            var deadPeasants =  Villages.FirstOrDefault(x => x.VillageName == args[1]).KillPeasants(rebelsCount);
            attacks++;
